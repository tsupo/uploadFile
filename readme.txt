uploadFile -- XML-RPC API を使ってココログにファイルをアップロードするプログラム
  ...
  ユーザ名、パスワードについて
    2009年6月2日から、ココログの管理画面へのログインは、ココログアカウントではなく、@nifty ID または @nifty
    ユーザ名を使うようになりました。本ツールで使う「ユーザ名」は @nifty ID または @nifty ユーザ名に変更して
    ください。「パスワード」についても、ココログアカウントに紐付いているパスワードではなく、@nifty ID に紐付
    いているパスワードに切り替えてください。ただし、2009年6月30日までは、XML-RPC API で使う「ユーザ名」、
    「パスワード」はココログアカウントの方を使う必要があります(@nifty ID や @nifty ユーザ名 では、XML-RPC API
    が使えません)。

    2009年7月1日から、XML-RPC API で使う「ユーザ名」も @nifty ID または @nifty ユーザ名に切り替わりました。
    ただし、@nifty ユーザ名でのログインや XML-RPC API の実行はうまくいかないことがあるようです。@nifty ユーザ
    名を使ってのアップロードがうまくいかない場合は、@nifty ID の方をお使いください。「パスワード」についても、
    @nifty ID に紐付いているパスワードに切り替えてください(@nifty ID と @nifty ユーザ名には、両者に同一のパス
    ワードが紐付いています)。

    なお、2009年7月1日以降、.. へのアップロードやファイルの削除等、ココログの管理画面にログインする必要のある
    処理に関しては、うまく動かなくなってしまっています(現在、対応策を検討中です)。一方、XML-RPC API によるアッ
    プロードは問題なく実行できることを確認しています。
  ...
  制限事項:
    ...
    2008年9月2日以降、ココログの仕様変更により、最大1MBまでのサイズのファイルしかアップロードできなくなってしま
    いました。
  ...
  更新情報:
    Ver 1.43  2009年  7月 3日 (1) @nifty ID によるアップロードが問題なくできることを確認
                              (2) .. へのアップロードやファイルの削除等、ココログの管理画面
                                  にログインする必要のある処理が、うまく動かなくなっている
                                  (2009年6月30日の @nifty 内サービスのシングルサインオン移
                                  行に伴うログイン手順の変更に対応し切れていないため)。完全
                                  対応は、次の版以降になる予定
                              (3) ユーザ名、パスワードを変更しても、設定ファイルに保存されな
                                  いことがある不具合を修正
    Ver 1.42  2009年  5月22日 .. へのアップロード時の不具合2点(ファイルの中身がおかしくなる、
                              アップロードが成功しているにも関わらず、失敗しました、と表示さ
                              れる)を修正 [同梱の xmlRPC.dll の修正による対応]
...
$Header: /comm/uploadFile/readme.txt 3     09/07/07 1:51 tsupo $
